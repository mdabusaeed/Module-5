...
class Grettings(View):
    grettings = 'Have a nice day'
  
    def get(self,request):
        return HttpResponse(self.grettings)
...
class UpdateTask(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        task_id = self.kwargs.get('id')  # 🔹 Get the ID properly
        print("Received ID:", task_id)  # 🔹 Debugging step
    ...
    def post(self, request, *args, **kwargs):
        task_id = self.kwargs.get('id')  # 🔹 Ensure correct ID
        task = get_object_or_404(Task, id=task_id)

        task_form = TaskModelForm(request.POST, instance=task)
        task_detail_form = TaskDetailModelForm(request.POST, instance=task.details if task.details else None)
        ...
        if task_form.is_valid() and task_detail_form.is_valid():
            ...
            messages.success(request, "Task Updated Successfully")
            return redirect(reverse('update-task', kwargs={'id': task_id}))
...
class UpdateView(UpdateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if task_form.is_valid() and task_detail_form.is_valid():
            ...
            messages.success(request, "Task Updated Successfully")
            return redirect(reverse('update-task', kwargs={'pk': self.object.pk}))

        return self.render_to_response('update-task', kwargs={'pk': self.object.pk})
...
class ViewProject(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        """Customize the queryset to include task count"""
        return Project.objects.annotate(num_tasks=Count('task')).order_by('num_tasks')
...
def task_details(request, task_id):
    ...
    return render(request, "task-detail.html", {"task": task ,"status_choices": status_choices})
